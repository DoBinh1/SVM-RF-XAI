...
- **Thực tế:** SVM + RBF kernel cho accuracy **~74–80%** trên CWRU khi **chia tập theo file** (đánh giá trung thực — thực nghiệm notebook: 74.6%; xem cảnh báo ở Mục 3)
...
- **Thực tế:** Accuracy **~78–83%** trên CWRU (chia theo file — thực nghiệm notebook: ~78%), tương đương hoặc tốt hơn SVM
...
> ⚠️ **CẢNH BÁO QUAN TRỌNG — Accuracy "ảo" do rò rỉ dữ liệu (data leakage):**
>
> Rất nhiều báo cáo nêu accuracy **99–100%** trên CWRU. Con số này thường **thổi phồng** vì một lỗi phương pháp phổ biến:
>
> - Tín hiệu được cắt thành segment với **overlap 50%** → hai segment kề nhau dùng chung tới một nửa số mẫu.
> - Nếu chia train/test **ngẫu nhiên trên từng segment** (`train_test_split` thường), segment kề nhau (gần như trùng) lọt vào *cả* train lẫn test → mô hình "học thuộc" rồi "thi lại đề cũ".
>
> ✅ **Cách đánh giá trung thực:** chia tập theo **file nguồn** (`GroupShuffleSplit` theo file) — toàn bộ segment của một file chỉ nằm ở train **hoặc** test. Khắt khe hơn nữa: **chia theo mức tải** (train 0–2 HP, test 3 HP) để mô phỏng domain shift thực tế.
>
> 📉 Khi đánh giá đúng (chia theo file), accuracy tụt mạnh: thực nghiệm trong notebook khóa này cho **SVM ≈ 74.6%, Random Forest ≈ 78%** (so với mức ảo ~94–98% khi chia ngẫu nhiên). Khác tải còn thấp hơn nữa. Đây mới là con số **đáng tin** để ra quyết định triển khai. Notebook thực hành (Tutorial 04/05) đã được sửa để chia theo file.

| Tiêu chí | SVM (RBF kernel) | Random Forest |
|---|---|---|
| **Độ chính xác** (chia theo file) | ~74–80% (thực nghiệm 74.6%) | ~78–83% (thực nghiệm ~78%) |
